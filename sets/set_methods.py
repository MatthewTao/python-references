...
def add_remove_items():
    data_scientist, data_engineer = (
        initialise_sets()["data_scientist"],
        initialise_sets()["data_engineer"],
    )
    ...
    data_engineer.discard(
        "Scala"
    )  # Doesn't raise any errors if the item doesn't already exist
    ...
    data_engineer.update(
        additional_skills
    )  # Add the items that don't already exist into the set
...
def set_operations():
    ...
    different_skills = data_engineer.difference(
        data_scientist
    )  # returns the items in data_engineer, but not in other
    print(different_skills)

    all_different_skills = data_engineer.symmetric_difference(
        data_scientist
...
def set_comprehension():
    result = {
        skill
        for skill in ["GIT", "PYTHON", "SQL"]
        if skill not in {"GIT", "PYTHON", "JAVA"}
    }
    print(result)
